...
const obtenerActores = async (req, res) => {
    try {
        const actores = await dbPrincipal.Actor.findAll({ order: [['id', 'DESC']] });
        res.json(actores);
    } catch (error) {
...
const obtenerActorPorId = async (req, res) => {
    const { id } = req.params;
    try {
        const actor = await dbPrincipal.Actor.findByPk(id);
...
const crearActor = async (req, res) => {
    ...
    try {
        const nuevoActor = await dbPrincipal.Actor.create({ nombre, imagen_url });
